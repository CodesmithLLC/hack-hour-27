...
function getPINs(observed) {
  ...
  let output = [];
  adjacents[observed[0]].forEach( elem => output.push(elem));
  if (observed.length > 1) {
    for (let i = 1; i < observed.length; i++) {
      let char = observed[i];
      let adjacentsArray = adjacents[char];
      let newOutput = [];
      adjacentsArray.forEach( adjNum => {
        output.forEach( elem => newOutput.push(elem + adjNum))
      })
      output = [...newOutput]
    }
  }
...
console.log(getPINs('1337897620'))
